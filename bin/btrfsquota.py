...
for line in subprocess.check_output(cmd).splitlines():
    args = str(line,encoding='utf8').strip().split(' ')
...
for line in subprocess.check_output(cmd).splitlines():
    args = [x for x in str(line,encoding='utf8').strip().split(' ') if len(x)>0]
    try:
    ...
    try:
        try:
            total = args[-2]#"%02.2f" % (float(args[1]) / multiplicator)
            unshared = args[-1]#"%02.2f" % (float(args[2]) / multiplicator)
        except ValueError:
            continue
        """print("%s\t%s\t%s%s %s%s" % (
        ...
        subvolume_name = (subvolume_name[:37] + '...') if len(subvolume_name) > 40 else subvolume_name
        print("%s\t%s\t%s %s" %( subvolume_name.ljust(40),args[0],total.rjust(15),unshared.rjust(15)))
    except IndexError:
        pass
